#!/usr/bin/env bash

# Locația fișierului de log
LOGFILE="/home/PersonalPortofolio/webhook.log"

# Redirecționăm atât stdout cât și stderr către fișierul de log
# Astfel, orice se afișează în acest script va fi scris în webhook.log
exec &>> "$LOGFILE"

# Activăm "set -x" pentru a afișa comenzile pe care le executăm
# (împreună cu argumentele lor). Asta ajută la debug și log.
set -x

echo -e "\n[$(date)] -- Webhook script START --"

# IMPORTANT: asigură-te că ai instalat openssl pentru a calcula HMAC.

# 1. Citește tot input-ul din stdin.
read_all_input() {
  local data
  data=$(cat)
  echo "$data"
}

process_request() {
  local raw_request="$1"

  # Spargem la prima aparitie a "\r\n\r\n" (sfârșitul headerelor HTTP)
  local headers body
  headers="$(echo "$raw_request" | sed -E '1,/^\r$/!d')"
  body="$(echo "$raw_request" | sed -E '1,/^\r$/d')"

  # Găsim semnătura "X-Hub-Signature-256"
  local signature_header
  signature_header="$(echo "$headers" | grep -i '^X-Hub-Signature-256:' | head -n1 | sed -E 's/^[^:]+:\s*//I')"
  signature_header="$(echo "$signature_header" | tr -d '\r')"

  if [ -z "$signature_header" ]; then
    send_http_response "400 Bad Request" "Missing X-Hub-Signature-256"
    echo "[$(date)] -- Missing X-Hub-Signature-256 --"
    exit 0
  fi

  # Extragem doar hash-ul după "sha256="
  local signature_value="${signature_header#sha256=}"

  # Importăm secretul
  source /home/PersonalPortofolio/.env

  # Calculăm semnătura locală
  local expected
  expected="$(printf "%s" "$body" | openssl dgst -sha256 -hmac "$WEBHOOK_SECRET" | cut -d ' ' -f2)"

  if [ "$expected" != "$signature_value" ]; then
    send_http_response "401 Unauthorized" "Invalid signature"
    echo "[$(date)] -- Invalid signature --"
    exit 0
  fi

  # Semnătura e validă -> rulăm docker compose
  echo "[$(date)] -- Signature valid --"

  cd /home/PersonalPortofolio || exit
  docker compose down
  docker compose up -d --build

  # Curățăm imaginile vechi, volume nefolosite etc.
  docker system prune -a --volumes -f

  # Trimitem notificarea pe Telegram
  /home/PersonalPortofolio/send_notification.sh

  # Trimitem răspunsul HTTP
  send_http_response "200 OK" "Deployment successful!"
  echo "[$(date)] -- Deployment successful! --"
}

send_http_response() {
  local status="$1"
  local message="$2"
  echo -e "HTTP/1.1 $status\r"
  echo -e "Content-Type: text/plain\r"
  echo -e "\r"
  echo -e "$message\r"
}

main() {
  # Citim tot input-ul
  local input
  input="$(read_all_input)"
  process_request "$input"
}

main

echo "[$(date)] -- Webhook script END --"
